...
class LetterSegmenter implements ISegmenter {
    ...
    /**
     * 在英文分词过程中，已经从英文转到阿拉伯数字
     */
    private boolean             turnToArabic;
    ...
    private boolean processMixLetter(AnalyzeContext context) {
        ...
        if (context.isBufferConsumed()) {
            if (this.start != -1 && this.end != -1) {
                //缓冲以读完，输出词元
                Lexeme newLexeme = new Lexeme(context.getBufferOffset(), this.start, this.end
                                                                                     - this.start
                                                                                     + 1,
                    Lexeme.TYPE_LETTER);
                context.addLexeme(newLexeme);
    ...
    private boolean processEnglishLetter(AnalyzeContext context) {
        ...
        if (this.englishStart == -1) {//当前的分词器尚未开始处理英文字符	
            ...
            if (CharacterUtil.CHAR_ENGLISH == context.getCurrentCharType()
                ...
                //英文第一次碰到数字，先将之前的英文输出
                if (CharacterUtil.CHAR_ARABIC == context.getCurrentCharType() && !turnToArabic) {
                ...
                //当出现数字后，再次出现英文，则此次英文分词结束
                if (CharacterUtil.CHAR_ENGLISH == context.getCurrentCharType() && turnToArabic) {
                    Lexeme newLexeme = new Lexeme(context.getBufferOffset(), this.englishStart,
                        this.englishEnd - this.englishStart + 1, Lexeme.TYPE_LETTER);
                    context.addLexeme(newLexeme);
                    this.englishStart = -1;
                    this.englishEnd = -1;
                } else {
                ...
                //遇到非English和阿拉伯字符,输出词元
                int letterType = Lexeme.TYPE_ENGLISH;
                if (turnToArabic) {
                    letterType = Lexeme.TYPE_LETTER;
                }
                Lexeme newLexeme = new Lexeme(context.getBufferOffset(), this.englishStart,
                    this.englishEnd - this.englishStart + 1, letterType);
                context.addLexeme(newLexeme);
                this.englishStart = -1;
                this.englishEnd = -1;
                turnToArabic = false;
    ...
    private boolean processArabicLetter(AnalyzeContext context) {
        ...
        if (this.arabicStart == -1) {//当前的分词器尚未开始处理数字字符	
            ...
            if (CharacterUtil.CHAR_ARABIC == context.getCurrentCharType()) {
                ...
                context.addLexeme(newLexeme);
                //数字输出结果后，碰到英文单词，多输出一个字符
                if (CharacterUtil.CHAR_ENGLISH == context.getCurrentCharType()) {
                    this.arabicEnd = context.getCursor();
                    Lexeme arabicEndWithLetterLexeme = new Lexeme(context.getBufferOffset(),
                        this.arabicStart, this.arabicEnd - this.arabicStart + 1,
                        Lexeme.TYPE_LETTER);
                    context.addLexeme(arabicEndWithLetterLexeme);
                }
                this.arabicStart = -1;
                this.arabicEnd = -1;
            }
